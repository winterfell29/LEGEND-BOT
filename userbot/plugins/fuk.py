...
from userbot import bot as legendx
from userbot.LEGEND import fetch_feds
...
@legendx.on(admin_cmd(pattern="fadd ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    nolol = 0
...
@legend.on(admin_cmd(pattern="frm ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    lol_s = event.pattern_match.group(1)
...
@legendx.on(admin_cmd(pattern="fban"))
async def _(event):
    if event.fwd_from:
        return
...
@legendx.on(admin_cmd(pattern="unfban ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    lol_s = event.pattern_match.group(1)
